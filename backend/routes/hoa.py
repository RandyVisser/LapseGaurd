...
async def _compliance_status_by_tenant(
    ...
    # Per-tenant rental context — a flagged owner unit also needs a lease on file
    # and (optionally) a rental endorsement on its HO-6.
    rental_ctx: dict = {}
    for c in await conn.fetch(
        """SELECT t.id AS tenant_id, u.is_rental, u.parent_unit_id,
                  (u.lease_document_url IS NOT NULL) AS has_lease, u.lease_extracted,
                  EXISTS(SELECT 1 FROM units cu JOIN tenants ct ON ct.unit_id = cu.id
                         JOIN policies cp ON cp.tenant_id = ct.id
                         WHERE cu.parent_unit_id = u.id) AS renter_has_ho4
           FROM tenants t JOIN units u ON u.id = t.unit_id
           WHERE t.id = ANY($1::uuid[])""",
        tenant_ids,
    ):
        rental_ctx[c["tenant_id"]] = c
    by_tenant: dict = {}
    for r in rows:
            ...
            for p in current_policies:
                ...
                if hoa_reqs:
                    try:
                        ...
                        pass
            # Flagged owner unit (subrented): once they have a policy on file, also
            # require a lease and — if the association requires it — a rental
            # endorsement on the HO-6.
            ctx = rental_ctx.get(tid)
            if not found_non_compliant and ctx and ctx["is_rental"] and ctx["parent_unit_id"] is None and current_policies:
                if not ctx["has_lease"]:
                    found_non_compliant = True
                else:
                    if hoa_reqs and hoa_reqs.get("rental_endorsement_required"):
                        endorsed = any(
                            (_json.loads(p["extracted_data"]) if isinstance(p["extracted_data"], str)
                             else (p["extracted_data"] or {})).get("has_rental_endorsement")
                            for p in current_policies
                        )
                        if not endorsed:
                            found_non_compliant = True
                    # Renter must carry an HO-4 (when the association requires it)
                    if not found_non_compliant and hoa_reqs and hoa_reqs.get("ho4_required") and not ctx.get("renter_has_ho4"):
                        found_non_compliant = True
                    # Lease effective dates active + term meets minimum (when parsed)
                    if not found_non_compliant:
                        lease = ctx.get("lease_extracted")
                        lease = _json.loads(lease) if isinstance(lease, str) else (lease or {})
                        start, end = lease.get("lease_start"), lease.get("lease_end")
                        if start and end:
                            today = date.today().isoformat()
                            if not (start <= today <= end):
                                found_non_compliant = True  # lease not currently active
                            else:
                                min_days = (hoa_reqs or {}).get("lease_min_term_days")
                                if min_days:
                                    try:
                                        term = (date.fromisoformat(end) - date.fromisoformat(start)).days
                                        if term < int(min_days):
                                            found_non_compliant = True
                                    except (ValueError, TypeError):
                                        pass
...
_HOA_SEARCH_FIELDS = """
    ...
    COALESCE(h.fein, (SELECT u.fein FROM units u WHERE u.hoa_id = h.id AND u.fein IS NOT NULL LIMIT 1)) AS fein
"""
    ...
    updated = await conn.fetchrow(
        """UPDATE hoas SET
           ...
           RETURNING id, name, address, alert_lead_days, ho6_coverage_a_min, ho6_coverage_e_min, ho6_wind_required,
                     ...
                     COALESCE(fein, (SELECT u.fein FROM units u WHERE u.hoa_id = hoas.id AND u.fein IS NOT NULL LIMIT 1)) AS fein,
